						  ...
						  std::vector<int> const &column_index)
{
	...
	this->splitting_attribute_ = attributes[util::rand_int(0, attributes.size() - 1)];
	...
	left_child_ = std::make_shared<Tree>();
	left_child_->setParent_id(this->getParent_id());
	left_child_->iTree(left_node_data, dataset, this->depth_ + 1, max_height, stop_height);

	right_child_ = std::make_shared<Tree>();
...
double osu::ad::Tree::path_length(std::vector<double> &instance, bool check_missing_value)
{
	...
	if (this->left_child_ == nullptr || this->right_child_ == nullptr)
	{
	...
	if (Tree::range_check)
	{
		...
		if (instance_attribute_value < this->min_attribute_val_ && osu::ad::util::rand_double(instance_attribute_value, this->min_attribute_val_) < this->min_attribute_val_)
			return 1.0;
		if (instance_attribute_value > this->min_attribute_val_ && osu::ad::util::rand_double(instance_attribute_value, this->max_attribute_val_) > this->max_attribute_val_)
			return 1.0;
	}
