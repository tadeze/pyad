...
def main():
    ...
    parser.add_argument('-a','--adaptive', action='store_true',help='Train using adaptive forest')
    parser.add_argument('-r','--rotate', action='store_true',help='Rotate data if checked rotates')
    parser.add_argument('-p','--pathlength', action='store_true',help='Output all depths')
    parser.add_argument('-d','--depth', action='store_true',help='Output average depth')
    parser.add_argument('-j','--skiprow',help="If there is header skip skip first row.",action='store_true')
    parser.add_argument('-g','--gtcolumn',help='skip column',type=int)
    parser.add_argument('-c','--column',help='Column to include defualt use all column.E.g. 1-10 or 1,3')
    parser.add_argument('-m','--model',type=argparse.FileType('w'),help='Save trained model to file. Provide file name')
    parser.add_argument('-l','--load',type=argparse.FileType('r'),help='Load model from file. Required saved model name')
    ...
    input_data = np.loadtxt(args.input,usecols=col_set,skiprows=header,delimiter=',') #input_data.iloc[:,col_set]
    if args.testfile is not None:
        test_data = np.loadtxt(args.testfile.name,usecols=col_set,skiprows=header,delimiter=',')
    else:
    ...
    np.savetxt(args.output, result,delimiter=',',header=header_output,comments='')
