...
class Loda(object):
    ...
    NA = -9999.0
    def histogram_r(self, x, g1=1., g2=1., g3=-1., verbose=False):
    ...
    def get_bin_for_equal_hist(self, breaks, x):
        if x < breaks[0]:
            return 0
    ...
    def pdf_hist_equal_bins(self, x, h, minpdf=1e-8):
    ...
    def pdf_hist(self, x, h, minpdf=1e-8):
        if type(x) is np.ndarray:
            ...
            for j in range(n):
                ...
                i = self.get_bin_for_equal_hist(h.breaks, x[j])
                if i >= len(h.density):
            ...
            i = self.get_bin_for_equal_hist(h.breaks, x)
            if i >= len(h.density):
                i = len(h.density) - 1
    ...
    def get_random_proj(self, nproj, d, sp, keep=None, exclude=None):
        ...
        for i in range(nproj):
            w[:, i] = random.randn(d)
            if nzeros > 0:
                z = Rtools.sample(idxs, min(nzeros, len(idxs)))
    ...
    def build_proj_hist(self, a, w):
        d = Rtools.ncol(w)  # number of columns
        x = a.dot(w)
        hists = []
        for j in range(d):
            hists_j = self.histogram_r(x[:, j])
    ...
    def get_neg_ll(self, a, w, hist, inf_replace=np.nan):
        ...
        pdfs[:, 0] = self.pdf_hist_equal_bins(x, hist)
    ...
    def get_all_hist_pdfs(self, a, w, hists):
        ...
        for i in range(len(hists)):
            hpdfs[:, i] = self.pdf_hist(x[:, i], hists[i])
        return hpdfs
    # get all pdfs from individual histograms.
        ...
        for i in range(0,a.shape[0]):
            ...
            if len(miss_column)>0:
                ...
                for ix,ihist in enumerate(idx_miss):
                    hpdfs[i, ihist] = self.pdf_hist(x[ix], hists[ihist])
            else:
                x = a[i,:].dot(w)
                for ihist in range(len(hists)):
                    hpdfs[i, ihist] = self.pdf_hist(x[ihist], hists[ihist])
        return hpdfs
    ...
    def loda(train,test=None, sparsity=np.nan, mink=1, maxk=0, keep=None, exclude=None, original_dims=False,check_miss=False):
        ...
        assert Rtools.ncol(train)==Rtools.ncol(test)
        l = Rtools.nrow(train)
        d = Rtools.ncol(train)
        ...
        if original_dims:
            pvh = self.get_original_proj(train, maxk=maxk, sp=sp, keep=keep, exclude=exclude)
        else:
            pvh = self.get_best_proj(train, mink=mink, maxk=maxk, sp=sp, keep=keep, exclude=exclude)


        nll = self.get_neg_ll_all_hist(test, pvh.pvh.w, pvh.pvh.hists, inf_replace=np.nan,check_miss=check_miss)
    ...
    def train(train, sparsity=np.nan, mink=1, maxk=0, keep=None, exclude=None,
        ...
        if original_dims:
            pvh = Rtools.get_original_proj(train, maxk=maxk, sp=sp, keep=keep, exclude=exclude)
        else:
            pvh = Rtools.get_best_proj(train, mink=mink, maxk=maxk, sp=sp, keep=keep, exclude=exclude)
        return pvh
